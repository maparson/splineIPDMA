    ...
    ) 
{
  ...
  for(i in studyIDs){
    ...
    if(linearCheck == FALSE) {
      # Get the knots within the range of study-level x
      knotLoci <- knotLoc[knotLoc >= min(testDati %>% pull(!!time.var)) &
                            knotLoc <= max(testDati %>% pull(!!time.var))]
      spline1 <- testDati %>% pull(!!time.var) %>% ns(knots = knotLoci)
      knotNi <- length(knotLoci)
      #--- remove matrix attributes
      spline2 <- lapply(spline1, function(x) { attributes(x) <- NULL; x })
      spline3 <- matrix(unlist(spline2), ncol = knotNi+1)
    } else {
    ...
    if(linearCheck == FALSE) {
      splineVars <- paste("V", 1:(length(knotLoci)+1), sep = "")
    } else {
      splineVars <- c("V1")
    }
    ...
    fit1 <- glm(data = testDatiSpline,
                ...
                family = "binomial")
    #--- check if singular or should be linear, if so then do linear model
    if(sum(is.na(coef(fit1))) > 0 | linearCheck == TRUE) {
      fit1 <- glm(data = testDatiSpline,
                  formula = paste("outcome ~ ", "as.numeric(",time.var,")"),
                  family = "binomial")
      linearCheck = TRUE
    }
    ...
    if(linearCheck == FALSE) {
      ...
      colnames(basis1) <- paste("V", 0:(length(knotLoci)+1), sep = "")
    } else {
      basis1 <- data.frame(cbind(rep(1,length(TimepointsGraph)), TimepointsGraph))
      colnames(basis1) <- c("V0", time.var)
    }
  ...
  sigma2 <- finalMat %>% group_by_at(time.var) %>% summarise(sigma2x = var(pred))
