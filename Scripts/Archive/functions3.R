  ...
  function(
    ...
    if (outcomeType == "binary") {
      if(condProbScale == TRUE) {
        ...
        condProbWithin[1] <- 1-((1-condProbWithin[1])*condProbOdds)
      }
      ...
      simOutcomesTibble = tibble(
        ...
        mutate(change = case_when(
          timePoint == 1 & outcome == 0 ~ as.numeric(rbernoulli(n = 1, p = 1-condProbWithin[1])),
          timePoint == 1 & outcome == 1 ~ as.numeric(rbernoulli(n = 1, p = 1-condProbWithin[2])),          
          lag(outcome,1) == 0 ~ as.numeric(rbernoulli(n = 1, p = 1-condProbWithin[1])),
          lag(outcome,1) == 1 ~ as.numeric(rbernoulli(n = 1, p = 1-condProbWithin[2]))
        )) %>%
