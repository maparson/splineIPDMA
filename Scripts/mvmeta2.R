  ...
  ) 
{
    ...
    # Get the overall basis vector
    
    if(nPowers == 2) {
      if(powersAll[1] != powersAll[2]) { # If the powers are different, just add em
        BasisTibbleOverall <- tibble( # Get the basis matrix including the intercept for matrix algebra
          intercept = 1,
          fp1 = FPone(FP = powersAll[1], beta = 1, x = TimepointsGraphOverall),
          fp2 = FPone(FP = powersAll[2], beta = 1, x = TimepointsGraphOverall),         
        )
      } else { # If the powers are the same then we need to multiply the second one by log
        BasisTibbleOverall <- tibble( # Get the basis matrix including the intercept for matrix algebra
          intercept = 1,
          fp1 = FPone(FP = powersAll[1], beta = 1, x = TimepointsGraphOverall),
          fp2 = log(TimepointsGraphOverall)*FPone(FP = powersAll[2], beta = 1, x = TimepointsGraphOverall),         
        )          
      }
    } else {
      BasisTibbleOverall <- tibble( # Get the basis matrix including the intercept for matrix algebra
    ...
    BasisMatrixOverall <- as.matrix(BasisTibbleOverall)
    ...
    for(i in studyIDs){
      ...
      BasisTibble <- tibble( # Get the basis matrix including the intercept for matrix algebra
          intercept = 1,
          fp1 = FPone(FP = powersAll[1], beta = 1, x = TimepointsGraph),
          fp2 = FPone(FP = powersAll[2], beta = 1, x = TimepointsGraph),         
        )
        } else { # If the powers are the same then we need to multiply the second one by log
          BasisTibble <- tibble( # Get the basis matrix including the intercept for matrix algebra
            intercept = 1,
            fp1 = FPone(FP = powersAll[1], beta = 1, x = TimepointsGraph),
            fp2 = log(TimepointsGraph)*FPone(FP = powersAll[2], beta = 1, x = TimepointsGraph),         
          )          
        }
      } else {
        BasisTibble <- tibble( # Get the basis matrix including the intercept for matrix algebra
    ...
    preds <- as.matrix(BasisMatrixOverall) %*% betaHat
    outcome <- fit1$family$linkinv(preds)
    
    x1 <- as.matrix(BasisMatrixOverall)
    si <- apply(x1, 1, function(x) {t(x) %*% icovsumInv %*% x})
    sei <- sqrt(si)
    loweri <- fit1$family$linkinv(as.matrix(BasisMatrixOverall) %*% betaHat - 1.96*sei)
    upperi <- fit1$family$linkinv(as.matrix(BasisMatrixOverall) %*% betaHat + 1.96*sei)
